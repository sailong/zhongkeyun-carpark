...
class OnAction extends CommonController {
	...
	public function index()
	{
		...
		//出场sql
		$out_sql = ' and end_status is null';
		...
		if ($type) {
			...
			if ($type < 5) {
				...
				if ($card_list) {
					$str = 'card_id in ('.join(',', $card_ids).')'.$out_sql.', start_time desc';
					$session_list = $mSession->getSessionByCond($str, $offset, $length);
					$count = $mSession->getSessionCount($str);
				}
			} else {
				$str = 'start_time >'.$s_time.' and start_time <'.$e_time.$out_sql.', start_time desc';
				$session_list = $mSession->getSessionByCond($str, $offset, $length);
				$count = $mSession->getSessionCount($str);
			}
			...
			if (intval($admin_id)) {
				...
				if ($doors) {
					...
					$str = join('or', $post_sql).$out_sql.', start_time desc';
					$session_list = $mSession->getSessionByCond($str, $offset, $length);
					$count = $mSession->getSessionCount($str);
				}
			} else {
