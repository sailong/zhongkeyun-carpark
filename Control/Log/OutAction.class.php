...
class OutAction extends CommonController {
	...
	public function index()
	{
		...
		//出场sql
		$out_sql = ' and end_status is not null';
		...
		if ($type) {
			...
			if ($type < 5) {
				...
				if ($card_list) {
					$str = 'card_id in ('.join(',', $card_ids).')'.$out_sql.', end_time desc';
					$session_list = $mSession->getSessionByCond($str, $offset, $length);
					$count = $mSession->getSessionCount($str);
				}
			} else {
				$str = 'end_time >'.$s_time.' and end_time <'.$e_time.$out_sql.', end_time desc';
				$session_list = $mSession->getSessionByCond($str, $offset, $length);
				$count = $mSession->getSessionCount($str);
			}
			...
			if (intval($admin_id)) {
				...
				if ($doors) {
					...
					foreach ($doors as $post) {
						$s_time = $post['s_time'];
						$e_time = $post['e_time'];
						$post_sql[] = ' (start_door_id in ('.$post['door'].') and end_time >'.$s_time.' and end_time <'.$e_time.')';
					}
					$str = join('or', $post_sql).$out_sql.', end_time desc';
					$session_list = $mSession->getSessionByCond($str, $offset, $length);
					$count = $mSession->getSessionCount($str);
				}
			} else {
		...
		foreach ($session_list as &$session) {
			...
			$post_end = $mAdmin->getAdminPostByDoorTime($session['end_door_id'], $session['end_time']);
			$session['end_admin_name'] = $post_end['admin_name'];
